...
class LogPolishTestCase(unittest.TestCase):
    ...
    def test_poll_filter_suppresses_open_file_state_checks(self):
        log_filter = main._SuppressPollLogs()
        for line in (
            '127.0.0.1 - - [30/Jul/2026 12:00:00] "GET /api/explorer/abc123/file/state HTTP/1.1" 200 -',
            '127.0.0.1 - - [30/Jul/2026 12:00:00] "GET /api/explorer/abc123/file/state?path=a.py&known=12:1.0 HTTP/1.1" 200 -',
        ):
            with self.subTest(line=line):
                self.assertFalse(log_filter.filter(_log_record(line)))

    def test_poll_filter_keeps_failed_open_file_state_checks(self):
        log_filter = main._SuppressPollLogs()
        for line in (
            '127.0.0.1 - - [30/Jul/2026 12:00:00] "GET /api/explorer/abc123/file/state?path=a.py HTTP/1.1" 400 -',
            # The full file read is a real request, never a suppressed poll.
            '127.0.0.1 - - [30/Jul/2026 12:00:00] "GET /api/explorer/abc123/file?path=a.py HTTP/1.1" 200 -',
        ):
            with self.subTest(line=line):
                self.assertTrue(log_filter.filter(_log_record(line)))
