...
- Added a Git sidebar for Local Repo and SSH file explorer panes, showing uncommitted changed files plus a collapsible commit graph with per-commit changed files and folder navigation, while the file Diff tab shows the currently opened file's read-only diff.
