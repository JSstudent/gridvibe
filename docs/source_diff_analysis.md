...
Status: Phases 1 and 2 implemented (2026-07-23); Phase 3 remains proposed.
...
### Phase 1 — Source highlighting — Done (2026-07-23)
...
- Vendored `web/static/vendor/highlight.min.js` — the Highlight.js 11.9.0
  "common" build with the `dos`, `powershell`, and `dockerfile` grammars
  appended so every language in `EXPLORER_HLJS_LANGUAGE` is covered. Documented
  with source URLs and the BSD-3-Clause license in
  `web/static/vendor/README.md`; loaded locally from `templates/terminals.html`.
- `explorerHighlightDocumentLines` (`web/static/js/explorer-viewer.js`)
  tokenizes the whole document once, then splits the highlighter output at
  newline boundaries into per-line styled runs. Each run carries its absolute
  character offset, so the existing offset-based search-mark machinery
  (`explorerCodeSpan` / `explorerMarkedEscHtml`) keeps working unchanged and
  multiline comments, strings, template literals, and embedded languages retain
  their state across line boundaries.
- `renderExplorerSourceLines` prefers the whole-document pass and falls back to
  the handwritten `highlightExplorerCode` lexer per line when Highlight.js is
  unavailable, rejects the language, throws, or the file is above the ~2 MiB
  plain-preview threshold. The log renderer and Markdown heading collapse stay
  on the fallback path and behave exactly as before.
- Token colors are explorer-scoped `.explorer-source-line-code .hljs-*` rules
  (light default plus `[data-explorer-theme="dark"]` overrides) in
  `web/static/css/terminals.css`. The grammar is always supplied explicitly;
  Highlight.js auto-detection is never used.
 
### Phase 2 — Precise diffs — Done (2026-07-23)
...
- Vendored `web/static/vendor/diff2html-ui-base.min.js` and
  `web/static/vendor/diff2html.min.css` (diff2html 3.4.48, MIT), documented in
  `web/static/vendor/README.md` and loaded locally from
  `templates/terminals.html`. GridVibe theme overrides live in `terminals.css`,
  so the vendored CSS is replaceable verbatim on upgrade.
- `renderExplorerDiff` now renders through `Diff2HtmlUI`, passing the pinned
  `hljs` instance, with `matching: 'words'` + `diffStyle: 'char'` for
  character-level intraline emphasis and LCS line matching (replacing FIFO
  pairing). `matchingMaxComparisons` and `maxLineLengthHighlight` are set
  explicitly. On any failure — missing assets, a throw, or a patch that parses
  to nothing — it falls back to the tolerant `renderExplorerSideBySideDiff`.
- The API's `truncated` flag is stored (`pane._explorerDiffTruncated`) and shown
  as a non-blocking banner ("Diff truncated to 256 KiB / 4,000 lines …").
- Search, per-tab font zoom, and scroll restoration are preserved with no
  changes to their code paths: diff search re-renders clean DOM and then marks
  text nodes (`markExplorerSearchInElement`), and zoom/scroll operate on the
  container, so they work over the Diff2Html DOM as well.

Post-implementation fixes (2026-07-23):

- Diff syntax colour: the Phase 1 Highlight.js palette was scoped only to
  `.explorer-source-line-code .hljs-*`, so Diff2Html's `.hljs-*` spans (under
  `.d2h-code-line-ctn`) rendered as uncoloured plain text. The palette
  selectors now use `:is(.explorer-source-line-code, .explorer-diff2html)` so
  Source and Diff share one palette.
- Stable split and independent horizontal scrolling: long lines no longer widen
  the combined diff or move its 50/50 split off screen. Source lines remain
  unwrapped, matching the file exactly, and a sticky horizontal scrollbar for
  each side stays at the visible viewport bottom when that side overflows.
  Scrolling one side does not move the other; scrollbar ranges are refreshed
  after rendering, resizing, search marking, and font zoom. Only the actual code
  span preserves source whitespace; Diff2Html's structural template whitespace
  is collapsed so it cannot create blank visual rows or detach gutter markers
  and line numbers from their source line. Line-number cells participate in
  their table row and are sticky only on the horizontal axis, so they move
  vertically with the code while remaining visible during side scrolling; the
  vendor padding reserved for absolutely positioned gutters is removed.
