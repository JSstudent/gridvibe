...
### Stage 2 — Make the instruction files true again (documentation only, zero code) — **DONE 2026-08-14**

> **Resolution (2026-08-14, Stage 2).** All six items closed in `CLAUDE.md` and
> `AGENTS.md`; no code touched. Two departed from the plan on the user's call:
>
> * **2.1 — the citation was removed, not repointed.** Repointing at *this*
>   document would have recreated the same failure mode one audit later (and
>   this file is under `docs/r&d/`, which the rule above forbids citing at all).
>   The paragraph now says audits are point-in-time records that are **never**
>   cited, and that the **Regression Guardrails list is the only live guardrail
>   contract** — an audit that produces or changes a guardrail must write the
>   rule itself into that list, in both files. The rule now survives without the
>   document, which is the whole lesson of §4.1/§5.1.
> * **2.6 — no change needed; the convention already exists and now holds.**
>   `docs/` currently contains exactly `logging_guide.md`, `testing_issues.md`,
>   `voice_guideline.md` and `images/` — both proposal documents §7.4 flagged are
>   gone from it. The existing pair of rules (the maintained/citable list, and
>   `docs/r&d/` as scratch) is the convention; dated proposals live in
>   `docs/r&d/`, and appear under `docs/` only for commit tracking, never as
>   contracts. The layout tree's `docs/` comment ("Public guides, audits,
>   testing issues") was corrected, since no audit lives there.
>
> The other four landed as written: **2.2** restores the allowed/not-allowed
> split and adds the earned clause — a DOM-free module gets a Node-executed
> behavioural test, and source-text assertions are for rendered markup and
> attribute hooks only. **2.3** gives the extraction rule a condition instead of
> an adjective: *the next change to the explorer tab strip extracts
> `explorer-tabs.js`* (with the pure-move test named), *then the next change to
> the Diff view extracts `explorer-diff.js`*. **2.4** — the audit's list of five
> was stale: `explorer-git-menu.js` and `test_explorer_git_menu.py` had since
> been added to the tree, while four other files were missing that the audit did
> not catch. Nine were added: `explorer-edit-find.js`,
> `explorer-edit-highlight.js`, `explorer-edit-overlay.js`,
> `explorer-tree-search.js`, `test_explorer_edit_find.py`,
> `test_explorer_edit_highlight.py`, `test_explorer_find.py`,
> `test_explorer_save_refresh.py`, `test_explorer_tree_fold.py`. **2.5** adds
> `GET /api/explorer/<id>/find` beside `search` in the read-only contract,
> stated as names-only and never file contents.
