...
- The explorer `Git` button opens a repository sidebar with uncommitted changed files and a collapsible commit graph. Sidebar file rows can open files or jump to their containing folder, and the file Diff tab shows the currently opened file's read-only side-by-side diff while preserving whitespace and syntax coloring.
...
- Local and SSH read-only file explorer panes with directory search, text/Markdown preview, syntax highlighting, per-pane editor font zoom, client-side file/diff search, and read-only Git status/diff awareness with a changed-file and commit-graph sidebar
