...
| **Git** | See branch and file status, inspect current or historical diffs, and stage, unstage, commit, publish, or discard changes. Diff views also support line and block undo. The commit graph has its own find box: type to highlight every matching commit message in the loaded graph, with a match counter and `Enter`/`Shift+Enter` (or the ↑/↓ buttons) to step through the hits and `Esc` to clear — the same controls as the `Ctrl+F` file find. It searches the commit subjects the graph is showing, so it finds what is on screen rather than replacing `git log --grep`. |
