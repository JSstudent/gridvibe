...
| **Select several** | `Ctrl`-click rows to add or remove them, `Shift`-click for a range. Right-click inside the selection and Copy, Cut, Delete, Download, and Copy path act on all of it, with one confirmation for the batch; right-click anywhere else drops back to that single row. A plain click still just opens the file. Rename stays single-entry. |
