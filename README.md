...
| **Git** | See branch and file status, inspect current or historical diffs, and stage, unstage, commit, publish, or discard changes. The sidebar starts linked to the explorer root chosen in the launcher or derived when a terminal enters explorer mode, so browsing into a subfolder does not reset the graph or change the target of Git actions. In the Graph header, the pin captures what the pane is showing as a new fixed Git scope — useful when the explorer root is above a repository — while the independent chain button follows what you browse. **Both go down to a single file**: pin a file and the graph, the change lists and every Git action show only that file, with the repository bar naming it; with follow on, opening a file scopes to it and opening a folder widens back to the folder, so switching tabs re-scopes as you go. The pin button means **pin here**: it is pressed only while you are browsing the pinned folder, where clicking clears the pin, and from anywhere else it wears a distinct outline, names the pinned folder in its tooltip, and moves the pin to the folder you are in with a single click — it never clears a pin you made somewhere else. **Clear pin**, on the pin's own row in the repository bar, is the named way to remove a pin, and it is there whenever a pin is — from the pinned folder itself as well as from anywhere else, so a pin on a folder that is collapsed, deleted, or outside the current root can always be cleared, and the control never moves out from under you as you browse. Both settings survive Save Workspace and restart; if both are enabled, turning follow off returns to the pinned folder. The one case where the scope is deliberately dropped is switching the pane to a terminal — the shell can walk to another repository entirely, so the fixed scope would be a lie — and follow-browsing goes with it, because reopening the explorer lands the pane wherever the shell walked to. The repository bar names the scope in use, `root` included, so a pin on the explorer root reads as a pin rather than as no pin; a pin on a folder that is inside no repository names that folder and offers **Clear pin**. A pin and a live Follow get a row each — pin first, then Follow — so it is always clear where the pin is, what Follow is tracking, and which of the two **Clear pin** applies to; while Follow is on the pin row dims and says it is overridden, since turning Follow off returns to it. You can also scope Git without browsing there first: right-click any folder or file — in the Files tree or in the Preview tab's directory listing — for **Pin Git here**, **Unpin Git** on the row that is the pin, and **Follow Git browsing**. The entry acts on one row only, so it disappears when several entries are selected, and a row inside no repository shows it greyed out with a tooltip saying why rather than hiding it. The pinned path is also marked in the Files tree, beside its open-in-folder button, so a pin you have navigated away from is still visible where you made it — a pin on the explorer root marks the **Files** header instead, and a pin whose folder is currently collapsed out of sight shows no mark, since the tree will not expand itself to reveal one. **Follow is marked the same way**, with the chain icon from its own button, on whichever row it is currently tracking — it moves as you browse, so the tree always shows what the graph is about without opening the sidebar. A row that is both pinned and followed wears both marks, since turning Follow off lands you back on the pin. The repository bar itself — repository, branch, and the pin and Follow rows — stays frozen at the top of the sidebar as you scroll, so what the change lists and the graph below are *about* is always on screen; **Publish**/**Push** sits just below it and scrolls away with everything else. **Stage All** and **Unstage All** act on every changed file at once; unlike **Discard All** beside them, neither asks for confirmation, because staging only moves entries in and out of the index and nothing in your working copy changes. All three obey the selected scope, so with a file scoped they act on that one file — their tooltips name the scope they will act in. **Commit** refuses rather than narrowing: if anything staged lies outside the scope it declines and asks you to widen the scope or unstage those paths, so it can never quietly commit part of what you staged. **Publish** stays branch-wide, since it pushes commits that already exist. Diff views also support line and block undo. The commit graph has its own find box behind the magnifying glass in the Graph header: type to highlight matching commit subjects, or turn on the hash button to find a full or abbreviated commit id, with a match counter and `Enter`/`Shift+Enter` (or the ↑/↓ buttons) to step through the hits — the same controls as the `Ctrl+F` file find — and `Esc` or the magnifier again to put it away. Find reads only the commits already loaded in Graph, so it finds what is on screen rather than issuing a separate `git log --grep` or id lookup — which is why a search that comes up empty says the graph is a *page* rather than the whole history, with **Show more** directly below it. Graph starts at the newest 60 commits in the selected scope and reads another 60 each time you press **Show more**, up to 300; once there is nothing more to load the button is replaced by the count it ended on, and at the 300 ceiling it says the graph stops there rather than that the history does. Right-click a commit row for a card with the full message, the author, when it was written (in the author's own time zone, with a relative age beside it), the full object id and any branch or tag decoration — with a copy button beside the message and another beside the id. It is wide enough to show the whole forty-character id on one line whatever the sidebar's width, opens above the row when there is no room below, and closes on `Esc`, on a click outside it, after a copy, or when you scroll the graph underneath it. `Alt`-click any commit row or chevron to collapse every expanded commit at once; the gesture never expands all. |
