...
| **Very large files & diffs** | Past ~20,000 lines or 4 MiB a file opens in a **large file view**: plain text, no syntax colour, line numbers, folding, change marks or overview ruler, and no find — a notice at the top of the pane names each one. Download and Edit still work. Large diffs degrade the same way in two steps, giving up intraline emphasis and then syntax colour, but always keeping side-by-side layout, line numbers, and line and block undo. Everything below the thresholds is unchanged. |
